...
if __name__ == '__main__':
    ...
    client.move(0.0, 0.0, 0.30, 0.0, 0.5)
    client.move(0.0, 0.0, 0.45, 0.0, 1.0)

    #############
    # FIXME - insert more move commands here...
    
    alts_up = np.array([.5, .5, .6, .7, .8, .9, 1, .5, .5, .6, .7, .8, .55, .65, .75])
    alts_down = np.array([.3, .25, .3, .25, .4, .35, .15, .1, .35, .25, .2, .1, .15, .3, .4])
    for i in range(15):
        # Move up to 0.50 meters:
        client.move(0.0, 0.0, alts_up[i], 0.0, 1.0)
        # Move down to 0.40 meters:
        client.move(0.0, 0.0, alts_down[i], 0.0, 1.0)
        # Return to hover at 0.45 meters:
        client.move(0.0, 0.0, 0.45, 0.0, 1.0)   
        print('iteration: ', i)
    ...
    client.write_data('force_data.json')
